...
class CommandToJsonTest(unittest.TestCase):
    ...
    def test_U15(self):
        ...
        self.assertEqual(output, {})
        output = get_json("okay glasses set note patient is sick end note")
        self.assertEqual(output['type'], "PROTOCOL")
        self.assertEqual(output['content'], "patient is sick")
        output = get_json("okay glasses start documentation bed 12 is empty end documentation")
        self.assertEqual(output['type'], "PROTOCOL")
    ...
    def test_U16(self):
        ...
        self.assertEqual(output, {})
        output = get_json("OK Glasses, set medication vitamins 1 pill end medication")
    ...
    def test_U17(self):
        ...
        output = get_json("OK Glasses, set medication vitamins 1 pill and lek 20 g and aspirin 2 liters at 2 PM end medication")
        self.assertEqual(output['time'].startswith("2"), True)
        self.assertEqual(output['content'][2]['quantity'], "2")
        self.assertEqual(len(output['content']), 3)
        output = get_json("OK Glasses, set medication nimm zwei 12 pieces at 4 AM end medication")
    ...
    def test_U20(self):
        output = get_json("ok glasses start message hello end message send it to John")
        self.assertEqual(output['type'], "MESSAGE")
        self.assertEqual(output['to'].title(), "john".title())
        self.assertEqual(output['content'], "hello")
        output = get_json("ok glasses start new message hello what's up patient 1 James is fine end new message send it to John Kowal Kowalski")
        self.assertEqual(output['type'], "MESSAGE")
